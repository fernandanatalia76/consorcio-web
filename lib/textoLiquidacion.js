...
function looksLikeRowStart(lineas, idx) {
  ...
  // FIX: en algunas filas el depto viene en su propia línea Y ADEMÁS
  // repetido, pegado sin espacio, al inicio de la línea del propietario
  // (ej. depto="110" en una línea, y la línea siguiente es
  // "110Barton Gabriel" en vez de directamente "Barton Gabriel"). Antes
  // se exigía que esa línea empezara con una LETRA — al empezar con el
  // número repetido, no se reconocía el corte de fila, y esos números
  // sueltos se sumaban por error a la fila anterior.
  if (esUF(l1) && idx + 2 < lineas.length && /^(?:\d+\s*)?[A-Za-zÁÉÍÓÚáéíóúñÑ]/.test(lineas[idx + 2])) {
    return true;
  }
  return false;
}
    ...
    if (mCoch) {
      ...
      if (/^\d{1,4}\s*(?:desc|cub)\s*$/i.test(sigLinea)) {
        ...
        if (mMerge) {
          ...
          propietario = lineas[i] || '';
          i++;
          // FIX: si el propietario viene con el número de depto repetido
          // pegado adelante (ej. "110Barton Gabriel" cuando depto="110"),
          // se lo saca para que quede solo el nombre.
          var deptoNum = String(depto).replace(/\D/g, '');
          if (deptoNum) {
            var reDup = new RegExp('^' + deptoNum + '\\s*');
            propietario = propietario.replace(reDup, '').trim();
          }
        }
      }
    } else {
