// ============================================================
// PARSER DE TEXTO PEGADO DE LIQUIDACIÓN (formato Word: un dato por
// línea, en el mismo orden de columnas que el resto del sistema).
// Incluye las UFs de cocheras ("desc" = descubierta, "cub" = cubierta),
// que vienen con el número de cochera pegado al UF en la misma línea
// (ej. "7 1 desc" o "20 16cub"), a diferencia de los departamentos
// donde UF y Depto suelen venir en líneas separadas.
// ============================================================
...
// UF + Depto de cochera pegados en la misma línea (ej. "7 1 desc", "20 16cub")
...
// ¿Arranca una fila nueva (departamento) en lineas[idx]? Mira hasta 2
// líneas adelante para no confundir un monto suelto (ej. el 2do
// vencimiento) con el inicio de la próxima fila.
function looksLikeRowStart(lineas, idx) {
  if (idx >= lineas.length) return false;
  if (RE_COCHERA.test(lineas[idx])) return true; // fila de cochera, auto-contenida
  if (!esUF(lineas[idx])) return false;
  if (idx + 1 >= lineas.length) return false;
  var l1 = lineas[idx + 1];

  if (RE_COCHERA.test(l1)) return false; // "idx" es en realidad un monto, no un UF

  // FIX: cuando cada celda del Word queda en su propia línea (en vez de
  // "UF Depto desc" todo junto), la celda de Depto de una cochera viene
  // SOLA con el formato "1 desc" o "16cub" — sin el propietario en la
  // misma línea. Hay que confirmar mirando la línea siguiente (el
  // propietario, que empieza con letra).
  if (/^\d{1,4}\s*(?:desc|cub)\s*$/i.test(l1)) {
    return idx + 2 < lineas.length && /^[A-Za-zÁÉÍÓÚáéíóúñÑ]/.test(lineas[idx + 2]);
  }

  // Depto + Propietario pegados en la misma línea (ej. "110 Barton Gabriel")
  if (/^\d+\s*(?:desc|cub)?\d*\s+[A-Za-zÁÉÍÓÚáéíóúñÑ]/i.test(l1)) return true;

  // Depto suelto (línea propia) — la línea SIGUIENTE (idx+2) debe ser
  // un nombre (empezar con letra) para confirmar que esto es una fila.
...
function parsearTextoLiquidacion(texto) {
  ...
  while (i < lineas.length) {
    ...
    if (mCoch) {
      ...
      // FIX: hay que chequear el patrón de cochera ("1 desc", "16cub")
      // ANTES que el merge genérico — si no, el merge genérico confunde
      // la palabra "desc"/"cub" con el nombre del propietario (matchea
      // como si "desc" fuera un nombre válido empezando con letra).
  ...
  if (!filas.length) {
    return { ok: false, error: 'No se pudo interpretar ninguna fila. Copiá el texto tal cual y avisá para ajustar.', textoCrudo: texto };
  }
