...
async function login(ssid, uf, pw, tipo) {
  ...
  // FIX: ahora cualquier fila con rol='admin' puede loguearse con SU
  // PROPIO usuario (no solo la fila literal "admin") — así se pueden
  // crear administradores adicionales, cada uno con su propia clave.
  var adminRow = usuarios0.find(function (u) { return u.uf === uf && u.rol === 'admin'; });
  if (adminRow) {
