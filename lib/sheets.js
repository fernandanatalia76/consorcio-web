...
async function leerGastos(ssid, mesFilter) {
  ...
  return rows.slice(2).filter(function (r) {
    ...
    // original.
    var driveUrl = String(r[15] || r[14] || '').trim();
    if (driveUrl && /\/file\/d\/[^/]+$/.test(driveUrl)) driveUrl += '/view';
    return { mes: r[0] || '', fecha: r[1] || '', proveedor: r[2] || '', cuit: r[3] || '', nroComprobante: r[4] || '', categoria: r[5] || '', importe: r[6] || '', formaPago: r[7] || '', fechaPago: r[8] || '', montoDebitado: r[9] || '', diferencia: r[10] || '', estado: r[11] || '', driveUrl: driveUrl };
  });
}
...
async function leerGastosExtraordinarios(ssid, mesFilter) {
  try {
    ...
    return rows.slice(2).filter(function (r) {
      ...
      var driveUrl = String(r[15] || r[14] || '').trim();
      if (driveUrl && /\/file\/d\/[^/]+$/.test(driveUrl)) driveUrl += '/view';
      return { mes: r[0] || '', fecha: r[1] || '', proveedor: r[2] || '', cuit: r[3] || '', nroComprobante: r[4] || '', categoria: r[5] || 'Extraordinario', importe: r[6] || '', formaPago: r[7] || '', fechaPago: r[8] || '', montoDebitado: r[9] || '', diferencia: r[10] || '', estado: r[11] || '', driveUrl: driveUrl };
    });
  } catch (e) {
