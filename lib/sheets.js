...
// Estructura: Mes | Saldo Inicial | Depositado | Rescatado | Saldo Final.
// FIX: en vez de asumir el nombre exacto "Cash Flow Extraordinarias" (que
// puede fallar por una diferencia sutil de caracteres, espacios, etc. y
// tirar "Unable to parse range"), primero se listan las solapas reales de
// la planilla y se busca la que coincida ignorando mayúsculas/espacios.
async function leerCashFlowExtraordinarias() {
  const sheets = await getSheets();
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
  ...
  var rows = await leerSolapa(real);
  if (rows.length < 3) return [];
  return rows.slice(2).filter(function (r) { return r[0]; }).map(function (r) {
