...
async function leerGastos(ssid, mesFilter) {
  ...
  return rows.slice(2).filter(function (r) {
    ...
    var driveUrlRaw = String(r[15] || r[14] || '').trim();
    var mFile = /\/file\/d\/([^/]+)/.exec(driveUrlRaw);
    var facturaFileId = mFile ? mFile[1] : '';
    return { mes: r[0] || '', fecha: r[1] || '', proveedor: r[2] || '', cuit: r[3] || '', nroComprobante: r[4] || '', categoria: r[5] || '', importe: r[6] || '', formaPago: r[7] || '', fechaPago: r[8] || '', montoDebitado: r[9] || '', diferencia: r[10] || '', estado: r[11] || '', detalle: r[12] || '', facturaFileId: facturaFileId };
  });
}
async function leerGastosExtraordinarios(ssid, mesFilter) {
  try {
    ...
    return rows.slice(2).filter(function (r) {
      ...
      var mFile = /\/file\/d\/([^/]+)/.exec(driveUrlRaw);
      var facturaFileId = mFile ? mFile[1] : '';
      return { mes: r[0] || '', fecha: r[1] || '', proveedor: r[2] || '', cuit: r[3] || '', nroComprobante: r[4] || '', categoria: r[5] || 'Extraordinario', importe: r[6] || '', formaPago: r[7] || '', fechaPago: r[8] || '', montoDebitado: r[9] || '', diferencia: r[10] || '', estado: r[11] || '', detalle: r[12] || '', facturaFileId: facturaFileId };
    });
  } catch (e) {
